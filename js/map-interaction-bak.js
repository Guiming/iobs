...
function initMap() {
	...
	polygon = new OpenLayers.Layer.Vector("Sighting Polygons", {
	...
	polyline = new OpenLayers.Layer.Vector("Activity Routes", {
	...
	if(mustWithinLashaArea){
		// study area
		studyAreaLyr = new OpenLayers.Layer.Vector("Mt. Lasha Area");	
	...
	*/
	...
	map.addLayers([polyline, polygon, ghyb, gphy, gmap]);	
	...
	drawPolygonControl.events.register("featureadded", this, function(e) {
		...
		if(mustWithinLashaArea){				
			...
			if(featureWithinPolygon(e.feature, studyAreaPolygon)){
				...
				var warningText = "Sorry, we cannot accept this sighting polygon as valid because it is not within Mt. Lasha area.";
	...
	var saveControl = new OpenLayers.Control.Button({
        ...
        trigger: function() {
			...
			// no drawings
				...
				openDialogMessage("Message", "You do not have any data to save.<br> At least one wildlife sighting polygon AND one activity route are required.");
			}
        },
